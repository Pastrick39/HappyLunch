    ...
    try:
        ...
        return {'用户名':网红名,       # text.split('"uniqueId":"')[1].split('"')[0]
            ...
            '用户安全ID':text.split('"secUid":"')[1].split('"')[0],
              ...
              '创建时间':datetime.fromtimestamp(int(text.split('"createTime":"')[1].split('"')[0])),
                ...
                '分享数':int(text.split('"shareCount":"')[1].split('"')[0]),
                  ...
                  '标题':标题,
                  '国家':text.split('"locationCreated":"')[1].split('"')[0],
                 'tag词':[] if "#" not in 标题 else list(map(lambda x:x.rstrip(),标题.split('#')[1:]))}
    except:
        # cprint('抓取失败，尝试调用EchoTik的rt接口')
...
#########################################################################
def 解析邮箱(text:str) -> str:
    # 给定一大串包含邮箱的文本（最多一个邮箱），返回这个邮箱
...
def 当前时间():
    # 以字符串形式返回当前时间:12:54:04
    return datetime.now().time().strftime('%H:%M:%S')

def 当前日期():
    # 以字符串形式返回当前日期:2025-10-16
    return date.today().strftime("%Y-%m-%d")

# def 进度条(i,s='='):
#     # 显示进度条
#     print(f"\r进度：{i}% [{s * i}{' ' * (100 - i)}]", end="")
#
# def 鼠标位置():
#     # 返回鼠标的当前位置，元组形式
...
def 启动360浏览器():
    # 启动360浏览器
    exe_browser = r"D:\软件安装\360SE\360se6\Application\360se.exe"  # 360 浏览器路径
    chromedriver_path = r"D:\软件安装\360SE\360se6\Application\chromedriver.exe"  # 对应版本的 chromedriver

    options = Options()
    options.binary_location = exe_browser
...
def 获取txt参数(参数名, txt_path=r'C:\py参数.txt'):
    # 指定txt文件路径，返回参数值。
    # txt文件的格式为：参数名：参数值，用中文“：”间隔，参数值可以是数字、字符串、布尔值等，参数之间用换行符隔开
    s=read_txt(txt_path)
    mylist=s.split('\n')
    for l in mylist:
        if l.split("：")[0]==参数名:
            return eval(l.split("：")[1])        # 使用eval()函数，保证数值返回的是int类型


def 剪贴板(操作, s=None):
    # 2025-11-07：读、写、清空剪贴板
    import pyperclip
    if 操作 == '读取':
